import Link from "next/link";
import type { Metadata } from "next";
import { ArrowLeft, ShieldCheck, Lock, AlertTriangle } from "lucide-react";
import { NexusIcon } from "@/components/icons/NexusIcon";
import { BrandName } from "@/components/brand/BrandName";


export const metadata: Metadata = {
  title: "Security Overview",
  description: "Security overview for NexusCanon Vendor Governance Platform",
};

// Route segment config for static generation with ISR
export const dynamic = "force-static";
export const revalidate = 3600;

export default function SecurityOverviewPage() {
  return (
    <div className="min-h-screen bg-background text-foreground font-sans">
      {/* Header */}
      <header className="border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-4 group">
            <div className="w-10 h-10 flex items-center justify-center">
              <NexusIcon size="lg" animated />
            </div>
            <div className="hidden md:block border-l border-border pl-4 group-hover:border-success-500/30 transition-colors duration-base">
              <BrandName variant="compact" />
            </div>
          </Link>
          <Link
            href="/docs"
            className="flex items-center gap-2 text-foreground-muted hover:text-foreground transition-colors text-sm font-brand"
          >
            <ArrowLeft className="w-4 h-4" />
            <span>Back to Docs</span>
          </Link>
        </div>
      </header>

      {/* Main Content */}
      <main className="max-w-4xl mx-auto px-6 md:px-12 py-16">
        <div className="mb-8">
          <Link
            href="/docs/security"
            className="text-[10px] font-brand text-foreground-muted uppercase tracking-[0.2em] hover:text-foreground transition-colors"
          >
            Security & Compliance
          </Link>
          <h1 className="text-5xl md:text-6xl font-serif text-foreground mb-4 mt-4">
            Security Overview
          </h1>
          <p className="text-lg text-foreground-muted font-normal leading-relaxed">
            Comprehensive security measures protecting your vendor governance data.
          </p>
        </div>

        <div className="prose prose-invert max-w-none space-y-8 text-foreground-muted font-normal leading-relaxed">
          <section>
            <h2 className="text-2xl font-serif text-foreground mb-4">Security Features</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="border border-border bg-background-elevated p-6">
                <Lock className="w-6 h-6 text-foreground-muted mb-3" />
                <h3 className="text-lg font-serif text-foreground mb-2">Zero-Trust Architecture</h3>
                <p className="text-sm text-foreground-muted">
                  Every access request is verified, regardless of origin
                </p>
              </div>
              <div className="border border-border bg-background-elevated p-6">
                <ShieldCheck className="w-6 h-6 text-foreground-muted mb-3" />
                <h3 className="text-lg font-serif text-foreground mb-2">Row Level Security</h3>
                <p className="text-sm text-foreground-muted">
                  Database-level access control for all data
                </p>
              </div>
              <div className="border border-border bg-background-elevated p-6">
                <AlertTriangle className="w-6 h-6 text-foreground-muted mb-3" />
                <h3 className="text-lg font-serif text-foreground mb-2">Continuous Monitoring</h3>
                <p className="text-sm text-foreground-muted">
                  24/7 security monitoring and threat detection
                </p>
              </div>
              <div className="border border-border bg-background-elevated p-6">
                <Lock className="w-6 h-6 text-foreground-muted mb-3" />
                <h3 className="text-lg font-serif text-foreground mb-2">Encryption</h3>
                <p className="text-sm text-foreground-muted">
                  End-to-end encryption for data in transit and at rest
                </p>
              </div>
            </div>
          </section>

          <section>
            <h2 className="text-2xl font-serif text-foreground mb-4">Compliance Certifications</h2>
            <p className="mb-4">
              NexusCanon maintains the highest compliance standards:
            </p>
            <div className="border border-border bg-background-elevated p-6 rounded-lg">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {['SOC 2 Type II', 'ISO 27001', 'GDPR', 'CCPA'].map((cert) => (
                  <div key={cert} className="text-center">
                    <p className="text-sm font-brand text-foreground-muted uppercase tracking-wider mb-1">{cert}</p>
                    <p className="text-xs text-foreground-subtle">Certified</p>
                  </div>
                ))}
              </div>
            </div>
          </section>

          <section>
            <h2 className="text-2xl font-serif text-foreground mb-4">Next Steps</h2>
            <div className="flex gap-3">
              <Link
                href="/docs/security/compliance"
                className="border border-border bg-background-elevated px-6 py-3 hover:bg-background-hover transition-colors text-sm font-brand uppercase tracking-wider"
              >
                Compliance Details
              </Link>
              <Link
                href="/docs/security/disaster-recovery"
                className="border border-border bg-background-elevated px-6 py-3 hover:bg-background-hover transition-colors text-sm font-brand uppercase tracking-wider"
              >
                Disaster Recovery
              </Link>
            </div>
          </section>
        </div>
      </main>

      {/* Footer */}
      <footer className="border-t border-border py-8 px-6 md:px-12 bg-background-elevated mt-16">
        <div className="max-w-[1600px] mx-auto">
          <div className="flex flex-col items-center gap-4">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 flex items-center justify-center">
                <NexusIcon size="lg" animated />
              </div>
              <div className="border-l border-border pl-4">
                <BrandName variant="compact" />
              </div>
            </div>
            <p className="text-xs text-foreground-subtle max-w-md text-center leading-relaxed">
              Institutional-grade vendor governance for the world's most regulated organizations.
            </p>
          </div>
        </div>
      </footer>
    </div>
  );
}


