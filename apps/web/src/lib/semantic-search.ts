 ...
 * 
 * IMPORTANT:
 * - Requires vector extension to be enabled
 * - Requires document_embeddings table to exist
 * - Embeddings must be generated for documents (via Edge Function)
 */
...
async function generateQueryEmbedding(query: string): Promise<number[]> {
  ...
  const { data, error } = await supabase.functions.invoke('generate-embedding', {
    body: {
      content: query,
      // Don't store this - it's just for search
    }
  })

  if (error || !data?.embedding) {
    throw new Error(`Failed to generate embedding: ${error?.message || 'No embedding returned'}`)
  }
...
export async function semanticSearchDocuments(
  ...
  try {
    ...
    // If semantic search fails (e.g., Edge Function not deployed), return empty
    // This allows graceful degradation to keyword search
...
export async function isSemanticSearchAvailable(): Promise<boolean> {
  try {
    ...
    // If table exists and has at least one embedding, semantic search is available
    return !error && (data !== null || (data === null && error === null))
  } catch {
    return false
  }
}
